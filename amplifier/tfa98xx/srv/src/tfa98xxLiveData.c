                                          ...
                                          nxpTfa98xx_LiveData_t * record)
{
        ...
        /* DRC */
        err = Tfa98xx_DspSupportDrc(handlesIn[idx], &record->DrcSupport );
        assert(err == Tfa98xx_Error_Ok);

        if ( record->DrcSupport ) {
        	int i;
        	for(i=0; i < (sizeof(record->drcState.GRblDrc[0])/sizeof(float));i++ ) {
        		record->drcState.GRhighDrc1[i] = stateInfo.drcState.GRhighDrc1[i];
        		record->drcState.GRhighDrc2[i] = stateInfo.drcState.GRhighDrc2[i];
        		record->drcState.GRmidDrc1[i] = stateInfo.drcState.GRmidDrc1[i];
        		record->drcState.GRmidDrc2[i] = stateInfo.drcState.GRmidDrc2[i];
        		record->drcState.GRlowDrc1[i] = stateInfo.drcState.GRlowDrc1[i];
        		record->drcState.GRlowDrc2[i] = stateInfo.drcState.GRlowDrc2[i];
        		record->drcState.GRpostDrc1[i] = stateInfo.drcState.GRpostDrc1[i];
        		record->drcState.GRpostDrc2[i] = stateInfo.drcState.GRpostDrc2[i];
        		record->drcState.GRblDrc[i] = stateInfo.drcState.GRblDrc[i];
        	}
        }
#endif
        /*
          ...
          */
         if (tfaRun_CheckAlgo(handlesIn[idx]))
         {
 			printf("statusRegister: 0x%02x\n", record->statusRegister);
         	tfaRun_SpeakerBoost(handlesIn[idx], 1); //force a reload
         	return nxpTfa98xx_Error_Fail;
         }

         switch(tfaRun_CheckEvents(record->statusRegister)) {//idx=0
     	case 1: //printf(">>>>>>>>>>repowered CF<<<<<<<<<<\n");
     			tfaRun_PowerCycleCF(handlesIn[idx]);
     		break;
     	case 2:
     			tfaRun_SpeakerBoost(handlesIn[idx], 1); //force a reload
     			return nxpTfa98xx_Error_Fail;
...
#ifndef WIN32 
        printf
            ("state: flags %s, agcGain %2.1llu\tlimGain %2.1llu\tsMax %2.1llu\tT %d\tX1 %2.1llu\tX2 %2.1llu\tRe %2.2llu\n",
...
void tfa98xxPrintRecordHeader(FILE * csv, int deltatime)
{
        ...
        fprintf(csv, "line,statusRegister,"
                "statusFlags,"
                "agcGain,"
                "limitGain,"
                "limitClip,"
        		...
        		"speakerResistance,"
        		"shortOnMips" "\n");
        // TODO do we need a header for drc?
}
...
static int tfa98xxLogStateLine(Tfa98xx_handle_t *handlesIn, side_t idx, int currentline, FILE *fp)
{
    ...
    fprintf(fp, "%d,0x%04x,0x%04x,%f,%f,%f,%f,%d,%d,%f,%f,%f,%d",
    		currentline,			 //d
    ...
    if ( record.DrcSupport ){
    	int i;
    	float *p= &record.drcState.GRhighDrc1[0] ;

    	/* just print all the floats */
...
int tfa98xxLogger( int interval, int loopcount)
{
	...
	// logfiles
	FILE *fp[3*MAX_DEVS], *out[2*MAX_DEVS];
...
//	FILE *gZmodelLeft,*gZmodelRight, *gXmodelLeft, *gXmodelRight;
	...
	if (  nxpTfa98xx_Error_Ok == nxpTfa98xxOpenLiveDataSlaves(handlesIn, tfa98xxI2cSlave, MAX_DEVS))
        do {
...
void tfa98xxPrintRecord(Tfa98xx_handle_t *handlesIn, FILE * csv, int idx)
{
    	...
    	tfa98xxLogStateLine(handlesIn, tfaLeft, currentline++, csv);// left
...
void tfa98xxPrintRecordStereo(Tfa98xx_handle_t *handlesIn, FILE * csv)
{
   ...
   {
      printf("Device %d\n", i);
      tfa98xxLogStateLine(handlesIn, i, currentline++, csv);
   }
}
