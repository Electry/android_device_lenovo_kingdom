...
#ifdef __KERNEL__
#define _ASSERT(e) if ((e)) pr_err("PrintAssert:%s (%s:%d)\n",__FUNCTION__,__FILE__,__LINE__)
#else
...
#if 0
	/* no check on length to allow also RAM image loading which */
	/* has bigger 'patch' files */
			    ...
			    Tfa98xx_OutputSel_t output_sel)
{
	...
	if (error == Tfa98xx_Error_Ok) {
		error =
		    Tfa98xx_WriteRegister16(handle, TFA98XX_I2S_SEL_REG, value);
	}
	return error;
}

Tfa98xx_Error_t
Tfa98xx_SelectI2SOutput(Tfa98xx_handle_t handle,
			     Tfa98xx_OutputSel_t output_sel, unsigned short value_set)
{
	Tfa98xx_Error_t error;
	unsigned short value;
	if (!handle_is_open(handle))
	...
	if (error == Tfa98xx_Error_Ok) {
		...
		switch (output_sel) {
		case TFA98XX_I2S_SEL_REG_SPKR:
			value &= ~(TFA98XX_I2S_SEL_REG_SPKR);
			value |= (value_set << TFA98XX_I2S_SEL_REG_SPKR_POS);	
		break;
		default:
			error = Tfa98xx_Error_Bad_Parameter;
		}
			     ...
			     Tfa98xx_OutputSel_t output_sel)
{
	...
	if (error == Tfa98xx_Error_Ok) {
		...
		case TFA98XX_I2S_SEL_REG_SPKR:
			value &= ~(TFA98XX_I2S_SEL_REG_SPKR);
			value |= (output_sel << TFA98XX_I2S_SEL_REG_SPKR_POS);	
		break;
		default:
			error = Tfa98xx_Error_Bad_Parameter;
		}
					  ...
					  const unsigned char *pSpeakerBytes)
{
...
#if 0
	{
		...
		for (i = 0; (i < handle_cnt) && (error == Tfa98xx_Error_Ok);
		     ++i) {
			error = Tfa98xx_DspSupportDrc(handles[i], 
							&bSupportDrc);
			...
			if (bSupportDrc) {
				...
				error = Tfa98xx_DspSetAgcGainInsert(
						handles[i],
				      ...
				      unsigned char *pConfigBytes)
{
	...
	if (pConfigBytes != 0) {
#if 0
		const unsigned char temp[TFA98XX_CONFIG_LENGTH +
...
Tfa98xx_Error_t Tfa98xx_SetMute(Tfa98xx_handle_t handle, Tfa98xx_Mute_t mute)
{
	...
	if (!handle_is_open(handle))
		return Tfa98xx_Error_NotOpen;
	error =
	    Tfa98xx_ReadRegister16(handle, TFA98XX_AUDIO_CTR, &audioctrl_value);
	if (error != Tfa98xx_Error_Ok)
		return error;
	error =
